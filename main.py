...
def insertAuthorsWithTempTable(json_file):
    start = time.time()
    #Needs id otherwise returns bad int, pandas bug ?
    chunks = pd.read_json(json_file, lines=True, chunksize=10000, dtype={'id': 'int64'})
    print("readlo to")
    conn = psycopg2.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    counter = 1
    command = """
    ...
    for chunk in chunks:
        if counter != 551:
            print("Inserting chunk " + str(counter))
            ...
            chunk = chunk[['id', 'name', 'username', 'description', 'followers_count', 'following_count', 'tweet_count',
                           'listed_count']]
            # chunk = chunk[['id', 'name', 'username', 'description']]
            #Replace new lines, blabla and delimiter
            chunk = chunk.replace(r'\n', ' ', regex=True)
            chunk = chunk.replace(r'\r', '', regex=True)
            chunk = chunk.replace(r';', ',', regex=True)
            chunk = chunk.replace(r'\x00', '\uFFFD', regex=True)
            chunk = chunk.replace(r'\\0', '\\\\', regex=True)
            chunk = chunk.replace(r'\\', '/', regex=True)
            chunk = chunk.replace(r'\.', 'hand', regex=True)
            chunk = chunk.replace({r'[^\x00-\x7F]+':''}, regex=True)
            ...
            buffer = io.StringIO()
            chunk.to_csv(buffer, header=False, index=False, sep=';', encoding='utf-8')
            chunk.to_csv('a.csv', header=False, index=False, sep=';', encoding='utf-8')
            buffer.seek(0)
            #print(buffer)
            cur.copy_from(buffer, 'authors_temp', sep=";")
            conn.commit()
        counter+= 1
    end = time.time()
    print(end - start)
    print("Copy from files complete, inserting into main table")
    command = """
        INSERT INTO authors
        SELECT *
        FROM authors_temp
    ...
    #cur.execute("DROP TABLE authors_temp")
    cur.close()
    # commit the changes
    conn.commit()
...
def insertAuthorsWithoutTempTable(json_file):
    start = time.time()
    #Needs id otherwise returns bad int, pandas bug ?
...
def prepareAuthors(json_file):
    start = time.time()
    # Needs id otherwise returns bad int, pandas bug ?
    timefile = open('createImportFilesAuthorsTime.csv', 'w')
...
def prepareConversations(json_file):
    ...
    counter = 1
    #cur.execute("TRUNCATE authors;")
    timefile = open('createImportFilesConversationsTime.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator = '\n')
    #usedIDs = []
    for chunk in chunks:
        smallstart = time.time()
...
def insertAuthors():
    ...
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    timefile = open('insertAuthors.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    command = """
        DROP TABLE IF EXISTS authors;
...
def insertConversations():
    start = time.time()
    # print("readlo to")
    timefile = open('insertConversations.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    # cur.execute("TRUNCATE authors;")
...
def insertHashtags():
    start = time.time()
    timefile = open('insertHashtags.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertLinks():
    start = time.time()
    timefile = open('insertLinks.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertAnnotations():
    start = time.time()
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    timefile = open('insertAnnotations.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    cur = conn.cursor()
    command = """
...
def insertConversationReferences():
    start = time.time()
    timefile = open('insertConversationReferences.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertContextDomains():
    start = time.time()
    timefile = open('insertContextDomains.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertContextEntities():
    start = time.time()
    timefile = open('insertContextEntities.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertContextAnnotations():
    start = time.time()
    timefile = open('insertContextAnnotations.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
def insertConversationsHashtags():
    start = time.time()
    timefile = open('insertHashtagsConversations.csv', 'w')
    timewriter = csv.writer(timefile, delimiter=';', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
    conn = psycopg.connect("dbname=postgres user=postgres password=neuhadnes3")
    cur = conn.cursor()
    command = """
...
if __name__ == '__main__':
    ...
    #prepareAuthors("C:\\Users\\vidaf\\PycharmProjects\\pythonProject\\Twitter Data\\authors.jsonl")
    #prepareConversations("C:\\Users\\vidaf\\PycharmProjects\\pythonProject\\Twitter Data\\conversations.jsonl")
    #insertAuthors()
    #insertConversations()
    # Hashtags HOTOVO, #TODO DELETE UNUSED HASHTAGS AFTER FK INSERT
    #insertHashtags()
    #insertLinks()
    #insertAnnotations()
    # conversation_references hotovo
    #insertConversationReferences()
    # insertContextDomains()
    #insertContextEntities()
    # Context annotations hotovo
    #insertContextAnnotations()
    # TODO ADDINGFK TAKES TOO LONG
    insertConversationsHashtags()
